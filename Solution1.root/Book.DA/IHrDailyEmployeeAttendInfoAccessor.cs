...
namespace Book.DA
{
    ...
    public partial interface IHrDailyEmployeeAttendInfoAccessor : IAccessor
    {
        ...
        int ReattenSave(Model.HrDailyEmployeeAttendInfo _hrEA);
        int UpdateSave_AnormalySalaryEditForm(Model.HrDailyEmployeeAttendInfo _hrDaily);

        void DeleteForChangeLeaveDateEmpHrDay(Model.Employee employee);

        double GetDayFactorByEmployeeIdAndDate(string EmployeeId, DateTime date);
    }
}
