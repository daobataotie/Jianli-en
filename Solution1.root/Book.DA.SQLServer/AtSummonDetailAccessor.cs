...
namespace Book.DA.SQLServer
{
    ...
    public partial class AtSummonDetailAccessor : EntityAccessor, IAtSummonDetailAccessor
    {
        ...
        {
            string sql = "select at.SummonDate,s.Id,s.SubjectName,a.Summary, case at.SummonCategory when 'Cash expenditures summons' then 0 else a.AMoney end as Income,case at.SummonCategory when 'Cash expenditures summons' then a.AMoney else 0 end as Pay, (case at.SummonCategory when 'Cash expenditures summons' then 0 else a.AMoney end -case at.SummonCategory when 'Cash expenditures summons' then a.AMoney else 0 end ) as YE from AtSummonDetail a left join AtSummon at on a.SummonId=at.SummonId left join AtAccountSubject s on a.SubjectId=s.SubjectId where at.SummonDate between '" + startDate.ToString("yyyy-MM-dd") + "' and '" + endDate.ToString("yyyy-MM-dd HH:mm:ss") + "' and (SummonCategory='Cash expenditures summons')order by at.SummonDate,at.SummonCategory  ";
        ...
        {
            string sql = " select (select Cast(sum(isnull(AMoney,0)) as decimal) from AtSummonDetail a left join AtAccountSubject s  on a.SubjectId=s.SubjectId left join AtSummon at on a.SummonId=at.SummonId where at.SummonCategory='Transfer summons' and s.SubjectName='現金' and at.SummonDate<'" + date.ToString("yyyy-MM-dd") + "') -  (select Cast(sum(isnull(AMoney,0)) as decimal) from AtSummonDetail a left join AtSummon at on a.SummonId=at.SummonId where at.SummonCategory='Cash expenditures summons' and at.SummonDate<'" + date.ToString("yyyy-MM-dd") + "') +(select TheBalance from AtAccountSubject where SubjectName='現金')";
