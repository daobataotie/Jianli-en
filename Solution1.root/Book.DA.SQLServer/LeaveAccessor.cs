...
namespace Book.DA.SQLServer
{
    ...
    public partial class LeaveAccessor : EntityAccessor, ILeaveAccessor
    {
        ...
        public int GetMonthIsGeZhouXiu(string empid, DateTime leavedate)
        {
            Hashtable ht = new Hashtable();
            ht.Add("empid", empid);
            ht.Add("year", leavedate.Year.ToString());
            ht.Add("month", leavedate.Month.ToString());

            return sqlmapper.QueryForObject<int>("Leave.GetMonthIsGeZhouXiu", ht);
        }

        public IList<Model.Leave> SelectByDate(DateTime date)
        {
            return sqlmapper.QueryForList<Model.Leave>("Leave.SelectByDate", date);
        }

        public string SelectYearLeaveCount(string p, int dateTime)
        {
            Hashtable ht = new Hashtable();
            ht.Add("Employeeid", p);
