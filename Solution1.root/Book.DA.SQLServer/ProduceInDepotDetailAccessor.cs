...
namespace Book.DA.SQLServer
{
    ...
    public partial class ProduceInDepotDetailAccessor : EntityAccessor, IProduceInDepotDetailAccessor
    {
        ...
        public IList<Book.Model.ProduceInDepotDetail> SelectList(string startPronoteHeader, string endPronoteHeader, DateTime startDate, DateTime endDate, Book.Model.Product product, Book.Model.WorkHouse work, Book.Model.Depot mDepot, Book.Model.DepotPosition mDepotPositioin, string id1, string id2, string cusxoid, Book.Model.Customer customer1, Book.Model.Customer customer2, int ProductState)
        {
            ...
            str.Append(" order by ProduceInDepotId desc");
        ...
        DataTable IProduceInDepotDetailAccessor.DTSelect_ChooseDefectRateCls(int DateType, DateTime StartDate, DateTime EndDate, string StartProduceInDepotId, string EndProduceInDepotId, Book.Model.Product StartProduct, Book.Model.Product EndProduct, string StartPronoteHeaderId, string EndPronoteHeaderId, Book.Model.WorkHouse StartWorkHouse, Book.Model.WorkHouse EndWorkHouse, Book.Model.Customer StartCustomer, Book.Model.Customer EndCustomer, bool attrJiLuFangShi, bool attrQiangHua, bool attrWuDu, bool attrWuQiangHuaWuDu, int attrProductStates, double RejectionRate, string RejectionRateCompare, bool EnableBLV, int attrOrderColumn, int attrOrderType, string StarInvoiceXOId, string EndInvoiceXOId, int InvoiceStates)
        {
            ...
            if (attrJiLuFangShi)
            {
                ...
                sbMain.Append("w.Workhousename,CASE p.ProductType WHEN 0 THEN 'Normal' WHEN 1 THEN 'Special' ELSE '' END AS ProductType,");
                sbMain.Append("CASE p.IsQiangHua WHEN 1 THEN 'Strengthen' ELSE (CASE p.IsFangWu WHEN 1 THEN 'Antifog' ELSE (CASE p.IsNoQiangFang WHEN 1 THEN 'No strengthen antifog' ELSE '' END) END ) END AS ProductWDQHua,");
                ...
                if (DateType == 0)
                {
                    sbContion.Append(" AND h.ProduceInDepotDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "'");
                }
                ...
                #region 查询集合以外满足条件的在加工单相关数据，即在该条件范围内加工生产，但不在该条件范围内入库
                ...
                if (DateType == 0)
                {
                    sbContion.Append(" AND (inh.ProduceInDepotDate < '" + StartDate.ToString("yyyy-MM-dd") + "' OR inh.ProduceInDepotDate > '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "')");
                }
                ...
                if (DateType == 0)
                {
                    sbContion.Append(" AND innh.ProduceInDepotDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "'");
                }
                if (!(string.IsNullOrEmpty(StartProduceInDepotId) && string.IsNullOrEmpty(EndProduceInDepotId)))
                {
                    if (!string.IsNullOrEmpty(StartProduceInDepotId) && !string.IsNullOrEmpty(EndProduceInDepotId))
            ...
            {
                #region 合并
                sbMain.Append("SELECT aa.*,case op.IsQiangHua WHEN 1 THEN 'Strengthen' else (case op.IsFangWU  WHEN 1 THEN 'Antifog' else(case op.IsNoQiangFang WHEN 1 THEN ' No strengthen antifog ' else '' end) END) END AS ProductWDQHua FROM (");
                ...
                sbMain.Append("CASE p.ProductType WHEN 0 THEN 'Normal' WHEN 1 THEN 'Special' else '' end AS ProductType,");
        ...
        public DataTable SUMDTSelect_ChooseDefectRateCls(int DateType, DateTime StartDate, DateTime EndDate, string StartProduceInDepotId, string EndProduceInDepotId, Book.Model.Product StartProduct, Book.Model.Product EndProduct, string StartPronoteHeaderId, string EndPronoteHeaderId, Book.Model.WorkHouse StartWorkHouse, Book.Model.WorkHouse EndWorkHouse, Book.Model.Customer StartCustomer, Book.Model.Customer EndCustomer, bool attrJiLuFangShi, bool attrQiangHua, bool attrWuDu, bool attrWuQiangHuaWuDu, int attrProductStates, double RejectionRate, string RejectionRateCompare, bool EnableBLV, int attrOrderColumn, int attrOrderType, string StarInvoiceXOId, string EndInvoiceXOId, int InvoiceStates)
        {
            ...
            sb.Append("CASE sum(d.ProceduresSum) WHEN 0 THEN '0%' ELSE RTrim(CONVERT(varchar,round(1 - isnull(sum(d.CheckOutSum),0)/isnull(sum(d.ProceduresSum),1),2)*100))+'%' END AS RejectionRate_1 ");
            ...
            if (DateType == 0)
            {
                sb.Append(" AND h.ProduceInDepotDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "'");
            }
            ...
            if (DateType == 0)
            {
                sb.Append(" AND (inh.ProduceInDepotDate < '" + StartDate.ToString("yyyy-MM-dd") + "' OR inh.ProduceInDepotDate > '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "')");
            }
            ...
            if (DateType == 0)
            {
                sb.Append(" AND innh.ProduceInDepotDate BETWEEN '" + StartDate.ToString("yyyy-MM-dd") + "' AND '" + EndDate.AddSeconds(-1).ToString("yyyy-MM-dd HH:mm:ss") + "'");
            }
        ...
        public DataTable PTSelect_ChooseDefectRateCls(DateTime StartDate, DateTime EndDate, string StartProduceInDepotId, string EndProduceInDepotId, Book.Model.Product StartProduct, Book.Model.Product EndProduct, string StartPronoteHeaderId, string EndPronoteHeaderId, Book.Model.WorkHouse StartWorkHouse, Book.Model.WorkHouse EndWorkHouse, Book.Model.Customer StartCustomer, Book.Model.Customer EndCustomer, bool attrJiLuFangShi, bool attrQiangHua, bool attrWuDu, bool attrWuQiangHuaWuDu, int attrProductStates, double RejectionRate, string RejectionRateCompare, bool EnableBLV, int attrOrderColumn, int attrOrderType)
        {
            ...
            if (attrJiLuFangShi)
            {
                ...
                sb.Append("(SELECT CASE ProceduresSum WHEN 0 THEN '0%' ELSE RTrim(CONVERT(varchar,round(1 - isnull(CheckOutSum,0)/isnull(ProceduresSum,1),2)*100))+'%' END FROM ProduceInDepotDetail WHERE ProduceInDepotDetail.ProduceInDepotDetailId = o.ProduceInDepotDetailId)  AS RejectionRate_1,");
                sb.Append("(SELECT Workhousename FROM WorkHouse WHERE WorkHouse.WorkHouseId=(SELECT WorkHouseId FROM ProduceInDepot WHERE ProduceInDepot.ProduceInDepotId=o.ProduceInDepotId)) AS Workhousename,");
                sb.Append("CASE (SELECT ProductType FROM Product WHERE Product.ProductId=o.ProductId) WHEN 0 THEN ' Normal ' else '' end AS ProductType,");
                sb.Append("CASE (SELECT IsQiangHua FROM Product WHERE Product.ProductId=o.ProductId) WHEN 1 THEN '  Strengthen  ' else");
                sb.Append("(CASE (SELECT IsFangWu FROM Product WHERE Product.ProductId=o.ProductId)  WHEN 1 THEN '  Antifog  ' else");
                sb.Append("(CASE (SELECT IsNoQiangFang FROM Product WHERE Product.ProductId=o.ProductId) WHEN 1 THEN ' No strengthen antifog ' else '' end) END) END AS ProductWDQHua,");
            ...
            else
            {
                ...
                sb.Append("ProductUnit AS ProductUnit,");
                sb.Append("isnull(sum(ProceduresSum),0) AS ProceduresSum,isnull(sum(CheckOutSum),0) AS CheckOutSum,");
                sb.Append("CASE sum(ProceduresSum) WHEN 0 THEN '0%' ELSE RTrim(CONVERT(varchar,round(1 - isnull(sum(CheckOutSum),0)/isnull(sum(ProceduresSum),1),2)*100))+'%' END AS RejectionRate_1,");
                sb.Append("CASE (SELECT ProductType FROM Product WHERE Product.ProductId=o.ProductId) WHEN 0 THEN ' Normal ' else '' end AS ProductType,");
                sb.Append("CASE (SELECT IsQiangHua FROM Product WHERE Product.ProductId=o.ProductId) WHEN 1 THEN ' Strengthen ' else");
                sb.Append("(CASE (SELECT IsFangWu FROM Product WHERE Product.ProductId=o.ProductId)  WHEN 1 THEN ' Antifog ' else");
                sb.Append("(CASE (SELECT IsNoQiangFang FROM Product WHERE Product.ProductId=o.ProductId) WHEN 1 THEN ' No strengthen antifog ' else '' end) END) END AS ProductWDQHua,");
        ...
        //生产进度总表
        public DataTable ProducePronotePlanReport(DateTime startdate, DateTime enddate, string pronoteheaderid, string xocustomerid, string invoiceCusId, string employeeid, string pronotetype, string workhouseid, string productid, bool? isJieAN)
        {
            StringBuilder sb = new StringBuilder();

            sb.Append("SELECT pdd.PronoteHeaderId AS JGDN,ph.InvoiceCusId AS KHDH,pro.ProductName AS HPMC,ph.InvoiceXODetailQuantity AS DDSL,");
            sb.Append("ph.DetailsSum AS SCSL,(SELECT TOP 1 InvoiceXO.InvoiceYjrq FROM InvoiceXO WHERE InvoiceXO.InvoiceId = ph.InvoiceXOId) AS KHJQ,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%射出%' THEN ppd.PronoteProceduresDate END AS SCJQ,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%射出%' THEN isnull(pdd.HeJiProceduresSum,0) ELSE '' END  AS SCHJSCSL,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%射出%' THEN isnull(pdd.HeJiCheckOutSum,0) ELSE '' END  AS SCHJHGSL,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%品管%' THEN ppd.PronoteProceduresDate END AS PGJQ,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%品管%' THEN isnull(pdd.HeJiProceduresSum,0) ELSE '' END  AS PGHJSCSL,");
            sb.Append("CASE WHEN wh.Workhousename LIKE '%品管%' THEN isnull(pdd.HeJiCheckOutSum,0) ELSE '' END  AS PGHJHGSL,");
            sb.Append("ppd.PronoteMachineId AS JQSB,");
            sb.Append("(SELECT '['+cast(pdmpro.ProductName AS varchar)+':'+ cast(isnull(pdm.PronoteQuantity,0) AS varchar) +'('+pdm.ProductUnit+')'+'],' FROM PronotedetailsMaterial pdm LEFT JOIN Product pdmpro ON pdmpro.ProductId = pdm.ProductId WHERE pdm.PronoteHeaderID = pdd.PronoteHeaderId FOR xml path('')) AS YL,");
            sb.Append("pro.ProductDescription AS SPMS,pid.ProduceInDepotDate as Date");
            //sb.Append("wh.Workhousename");
            sb.Append(" FROM ProduceInDepotDetail pdd ");
            sb.Append(" LEFT JOIN ProduceInDepot pid ON pid.ProduceInDepotId = pdd.ProduceInDepotId");
            sb.Append(" LEFT JOIN Product pro ON pro.ProductId = pdd.ProductId");
            sb.Append(" LEFT JOIN PronoteHeader ph ON pdd.PronoteHeaderId = ph.PronoteHeaderId");
            sb.Append(" LEFT JOIN PronoteProceduresDetail ppd ON ppd.PronoteHeaderID = ph.PronoteHeaderID");
            sb.Append(" LEFT JOIN WorkHouse wh ON wh.WorkHouseId = ppd.WorkHouseId");
            sb.Append(" WHERE 1 = 1");
            sb.Append(" AND ph.PronoteDate BETWEEN '" + startdate.ToString("yyyy-MM-dd") + "' AND '" + enddate.ToString("yyyy-MM-dd") + "'");
            if (!string.IsNullOrEmpty(pronoteheaderid))
                sb.Append(" AND ph.PronoteHeaderID = '" + pronoteheaderid + "'");
            if (!string.IsNullOrEmpty(invoiceCusId))
                sb.Append(" AND ph.InvoiceCusId = '" + invoiceCusId + "'");
            if (!string.IsNullOrEmpty(xocustomerid))
                sb.Append(" AND ph.InvoiceXOId IN (SELECT InvoiceXO.InvoiceId FROM InvoiceXO WHERE InvoiceXO.xocustomerId = '" + xocustomerid + "')");
            if (!string.IsNullOrEmpty(employeeid))
                sb.Append(" AND ph.Employee0Id = '" + employeeid + "'");
            if (!string.IsNullOrEmpty(workhouseid))
                sb.Append(" AND ph.WorkHouseId = '" + workhouseid + "'");
            if (!string.IsNullOrEmpty(productid))
                sb.Append(" AND ph.ProductId = '" + productid + "'");
            if (isJieAN.HasValue)
                sb.Append(" AND ph.IsClose = '" + (isJieAN.Value ? "1" : "0") + "'");
            sb.Append(" AND (wh.Workhousename LIKE '%射出%' OR wh.Workhousename LIKE '%品管%')");
            sb.Append(" order by ph.PronoteDate,pid.ProduceInDepotDate desc");

            return this.Query(sb.ToString(), 45, "PDDA").Tables["PDDA"];
        }

        public double GetTotalProceduresSumByPronote(string PronoteHeaderId)
        {
            return sqlmapper.QueryForObject<double>("ProduceInDepotDetail.GetTotalProceduresSumByPronote", PronoteHeaderId);
        }
    }
}
