...
namespace Book.DA.SQLServer
{
    ...
    public partial class MiscDataAccessor : Accessor, IMiscDataAccessor
    {
        ...
        public System.Data.DataTable SelectProductStock()
        {
            ...
            sda.SelectCommand.Connection = new System.Data.SqlClient.SqlConnection(sqlmapper.DataSource.ConnectionString);
            System.Data.DataTable table = new System.Data.DataTable();
            string sql = "select isnull((select DepotName from Depot where DepotId=s.DepotId),'------') as DepotName,p.productName as ProductName,p.ProductDescription as ProductDescription,p.Id as spid,isnull((select CnName from ProductUnit u where p.DepotUnitId = u.ProductUnitId and p.productid = s.productid),'------') as CnName,isnull((select Id from depotposition  where depotpositionId = s.depotpositionId),'------') as DepotPositionId,isnull(s.StockQuantity1,0) as Quantity from product p left join stock s on p.productId =s.productId";
            sda.SelectCommand.CommandText = sql;
            sda.Fill(table);
            return table;
        }
