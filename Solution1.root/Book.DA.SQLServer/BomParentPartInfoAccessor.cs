...
namespace Book.DA.SQLServer
{
    ...
    public partial class BomParentPartInfoAccessor : EntityAccessor, IBomParentPartInfoAccessor
    {
        ...
        {
            return SQLDB.DbHelperSQL.Query("select BomId ,Id,BomVersion,DefaultQuantity,(select employeename from employee e where employeeid=b.EmployeeAddId ) as EmployeeAddName ,(select id from product p where p.productid =b.productid) as  ProId,(select ProductName from product p where p.productid =b.productid) as  ProductName,(select CustomerProductName from product p where p.productid =b.productid) as  CustomerProductName,(select ProductName from product p where p.productid =b.productid) as  ProductName,(select CustomerProductName from product p where p.productid =b.productid) as  CustomerProductName,(SELECT CustomerShortName FROM Customer WHERE Customer.CustomerId=(SELECT customerid FROM product WHERE productid =b.ProductId)) as CustomerName,b.IsEdit from BomParentPartInfo  b where  b.productid not in(select productid from BomComponentInfo)  order by b.id desc ");
        ...
        {
            return SQLDB.DbHelperSQL.Query("select BomId ,Id,DefaultQuantity,BomVersion,(select employeename from employee e where employeeid=b.EmployeeAddId ) as EmployeeAddName ,(select id from product p where p.productid =b.productid) as  ProId,(select ProductName from product p where p.productid =b.productid) as  ProductName,(select CustomerProductName from product p where p.productid =b.productid) as  CustomerProductName,(select ProductName from product p where p.productid =b.productid) as  ProductName,(select CustomerProductName from product p where p.productid =b.productid) as  CustomerProductName,(SELECT CustomerShortName FROM Customer WHERE Customer.CustomerId=(SELECT customerid FROM product WHERE productid =b.ProductId)) as CustomerName,b.IsEdit from BomParentPartInfo  b  order by b.id desc ");
        }
