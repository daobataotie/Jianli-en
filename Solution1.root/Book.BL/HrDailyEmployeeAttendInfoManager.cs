...
namespace Book.BL
{
    ...
    public partial class HrDailyEmployeeAttendInfoManager
    {
        ...
        public void ReCheck(DateTime date, Model.Employee emp)
        {
            ...
            //已經自動考勤過了，所以必需再重新考勤
            if (limitDate >= date && emp.EmployeeJoinDate <= date)
            {
        ...
        public Model.HrDailyEmployeeAttendInfo Reatten_Controller(DateTime checkdate, Model.Employee emp)
        {
            ...
            //if (mNote.Contains("曠職"))
            //{
            //    Model.LeaveType mLeaveType;
            //    if (mNote.Contains("整日"))     //本日只有一条请假记录
            //    {
            //        mLeaveType = mLeaves[0].LeaveType;
            //        mDayFactor = mLeaveType.PayRate;
            //        mMonthFactor = mLeaveType.PayRate;
            //    }
            //    else
            //    {
            //        if (mNote.Contains("上半日"))
            //        {
        ...
        {
            return accessor.GetDayFactorByEmployeeIdAndDate(EmployeeId, date);
        }
    }
}