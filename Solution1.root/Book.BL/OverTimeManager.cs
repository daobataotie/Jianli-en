    ...
    public partial class OverTimeManager : BaseManager
    {
        ...
        public void InsertList(Model.OverTime overTime, IList<Model.Employee> emps)
        {
            for (int i = 0; i < emps.Count; i++)
            {
                ...
                {
                    if (emps[i].DailyPay > 0 && ot.EoverTime > 0 && ot.EoverTime <= 2)
                    {
                        ot.OverTimeFee = emps[i].DailyPay.Value / 8 * decimal.Parse(ot.EoverTime.ToString()) * decimal.Parse(1.33.ToString());
                    }
                    else if (emps[i].DailyPay > 0 && ot.EoverTime > 2)
                        ot.OverTimeFee = emps[i].DailyPay.Value / 8 * Convert.ToDecimal(2 * 1.33) + emps[i].DailyPay.Value / 8 * Convert.ToDecimal(ot.EoverTime - 2) * Convert.ToDecimal(1.66);
                ...
                else  //假日加班
                {
                    ...
                    {
                        ot.OverTimeFee = emps[i].DailyPay.Value / 8 * decimal.Parse(ot.EoverTime.ToString()) * 2;
                    }
                    if (emps[i].DailyPay == 0 && emps[i].MonthlyPay > 0)
                    {
